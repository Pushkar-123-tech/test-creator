...
export default function RecentTests() {
  ...
  useEffect(() => {
    const fetchTests = async () => {
      ...
      setLoading(true);
      try {
        const response = await aiClient.request(`/api/ai/test-cases?userId=${user.id}&limit=5`);
        if (response.success) {
  ...
  return (
    <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-bold text-white m-0">Recent Test Cases</h2>
            ...
            <div key={test.id || index} className="question-item flex items-center gap-3.5">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[13px] font-semibold text-[#e8e8f0] truncate">
                  {test.test_type.charAt(0).toUpperCase() + test.test_type.slice(1)} Test Case
                </div>
                <div className="text-[11px] text-[#66667a] mt-0.5 truncate">
                  {test.code.substring(0, 50)}...
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <div className="flex items-center gap-1.5">
                  <div className={`w-1.5 h-1.5 rounded-full ${getSeverityColor(test.severity || 'medium')}`}></div>
                  <span className="text-[11px] text-[#66667a] capitalize">{test.severity || 'Medium'}</span>
                </div>
